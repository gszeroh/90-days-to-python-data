"""Day 33: Seaborn — Statistical Visualization — Solutions
...
def categorical_comparison(df, x, y, title="Categorical Comparison"):
    ...
    sns.stripplot(data=df, x=x, y=y, color="black", alpha=0.3, size=3, ax=ax)
    ax.set_title(title)
    fig.tight_layout()
    return ax
...
def regression_plot(df, x, y, ci=95, title="Regression"):
    ...
    """
    fig, ax = plt.subplots()
    sns.regplot(data=df, x=x, y=y, ci=ci,
...
def pair_plot(df, numeric_cols, hue_col, diag_kind="kde"):
    ...
    """
    g = sns.pairplot(df, vars=numeric_cols, hue=hue_col,
                     diag_kind=diag_kind)
    return g
