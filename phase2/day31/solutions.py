"""Day 31: Matplotlib — Fundamentals — Solutions
...
def grouped_bar_chart(labels, group_data, group_names, title):
    ...
    for i, name in enumerate(group_names):
        ...
        ax.bar(x + offset, group_data[name], width, label=name)
...
def scatter_with_cmap(x, y, colors, title, cmap="viridis"):
    ...
    """
    fig, ax = plt.subplots()
    sc = ax.scatter(x, y, c=colors, alpha=0.7, cmap=cmap)
...
def multi_plot(x, y_line, y_scatter, hist_data):
    ...
    # (0, 1) Scatter plot
    axes[0, 1].scatter(x, y_scatter, alpha=0.6)
    axes[0, 1].set_title("Scatter Plot")

    # (1, 0) Histogram
    axes[1, 0].hist(hist_data, bins=20, edgecolor="black", alpha=0.7)
    axes[1, 0].set_title("Histogram")

    # (1, 1) Bar chart
    axes[1, 1].bar(["A", "B", "C", "D"], [4, 7, 1, 8])
